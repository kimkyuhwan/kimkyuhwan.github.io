...
정점을 가능한 많이 선택하되 선택된 정점끼리 서로 인접하지 않는 집합을 독립 집합(independent set) 이라고 부른다. 이 때, 가장 큰 독립 집합을 최대 독립 집합(maximum independent set)이라고 하며. 이 문제는 NP-Complete 문제이다. 

격자 그래프는 항상 이분 그래프로 변형할 수 있다.
...
f_k(x)
$$
값이 최소 값이 되는 x값을 구하는 것이 문제에서 요구하는 바이다.
